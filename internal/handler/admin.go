...
func (h *AdminHandler) ServeAdmin2(w http.ResponseWriter, r *http.Request) {
	data, err := fs.ReadFile(dashboard.FS, "admin2.html")
	if err != nil {
		http.Error(w, "admin2 page not found", http.StatusInternalServerError)
		return
	}
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Write(data)
