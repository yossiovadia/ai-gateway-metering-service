...
#
# Usage:
#   ./manage-keys.sh <email> <group> <admin=y|n> [key_name]
#
# Examples:
#   ./manage-keys.sh newuser@redhat.com octo-eng n
#   ./manage-keys.sh newuser@redhat.com octo-eng n jane-claude
#
# Behavior:
...
#   - Display name: after the user first logs in, add their first/last name
#     on the metering admin page (Display Names card) — the dashboard shows
...
usage() {
	echo "usage: $0 <email> <group> <admin=y|n> [key_name]" >&2
	exit 1
}
[ $# -ge 3 ] || usage
...
echo "Next: add the user's display name on the metering admin page (Display Names card)."
