...
- Where: `internal/handler` (middleware around the 6 handlers only —
  manager/user-scoped endpoints have their own authz scoping; include it
  in the key or leave those uncached in this phase).
- Memory-bounded: fixed-size LRU (say ≤512 entries; results are small
  JSON blobs). Per the house rule: bounded buffers, no unbounded maps.
